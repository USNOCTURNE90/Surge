import os, re, subprocess, json, ipaddress
...
root = Path(".")
# 排除列表
excludes = ['.git', '.github', 'Clash_Repo', 'SB_Repo', 'LICENSE', 'README.md']

# 获取所有规则文件
files = [f for f in root.glob('*') if f.is_file() and f.name not in excludes and f.suffix not in ['.py', '.yml', '.yaml']]

# 同步 Clash
c_repo = f"https://{token}@github.com/USNOCTURNE90/Clash.git"
subprocess.run(["git", "clone", c_repo, "Clash_Repo"], check=True)

# 同步 Sing-Box
s_repo = f"https://{token}@github.com/USNOCTURNE90/Sing-Box.git"
subprocess.run(["git", "clone", s_repo, "SB_Repo"], check=True)
...
for f_path in files:
    ...
    with open(Path("Clash_Repo") / f_path.name, "w", encoding="utf-8") as f:
        f.write("\n".join(clash_data))
    
    # 3. 生成 Sing-Box 文件
    sb_data = to_sb(new_lines)
    if sb_data:
        with open(Path("SB_Repo") / f"{f_path.name}.json", "w", encoding="utf-8") as f:
            json.dump(sb_data, f, indent=2, ensure_ascii=False)

# 提交更改
def push_repo(path, msg):
    subprocess.run(["git", "-C", path, "add", "."], check=True)
    res = subprocess.run(["git", "-C", path, "status", "--porcelain"], capture_output=True, text=True).stdout
    if res.strip():
...
push_repo("Clash_Repo", f"[AUTO_SYNC] Clash {get_time()}")
push_repo("SB_Repo", f"[AUTO_SYNC] Sing-Box {get_time()}")
