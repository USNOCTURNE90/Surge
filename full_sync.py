...
files = [f for f in root.glob('*') if f.is_file() and f.name not in excludes and f.suffix not in ['.py', '.yml', '.yaml', '.md']]

# 清理并克隆
for d in ["Clash_Repo", "SB_Repo"]:
...
subprocess.run(["git", "clone", c_url, "Clash_Repo"], check=True)
subprocess.run(["git", "clone", s_url, "SB_Repo"], check=True)
...
def push_repo(path, msg):
    ...
    if subprocess.run(["git", "-C", path, "status", "--porcelain"], capture_output=True, text=True).stdout.strip():
